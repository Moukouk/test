...
const mc = mysql.createConnection({
    host: 'localhost',//'5.196.64.114',
    user: 'root',//'root',
    password: '',//'argosnord-mysql',
    database: 'test'//'argos',
});
...
app.get('/', function (req, res) {
    return res.send({ error: true, message: 'hello2:' })
});
...
app.get('/todos', function (req, res) {
    ...
    /*mc.query('SELECT * FROM data_argus_alarms WHERE id>=100000', function (error, results, fields) {
        if (error) throw error;
        return res.send({ error: false, data: results, message: 'Todos list.' });
    });*/
});
